...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['event-organizer', 'social-media-promoter', 'user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<StudentInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/students` : null),
    () => getStudents({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Student',
      path: isTenantUser ? `/students/view/${data?.[0]?.id}` : '/students',
      entity: 'student',
      service: AccessServiceEnum.PROJECT,
      icon: FiUserCheck,
    },
    { name: 'Event', path: '/events', entity: 'event', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
    {
      name: 'Guest List',
      path: '/guest-lists',
      entity: 'guest_list',
    ...
    { name: 'Rsvp', path: '/rsvps', entity: 'rsvp', service: AccessServiceEnum.PROJECT, icon: FiCheckSquare },
    { name: 'Sos', path: '/sos', entity: 'sos', service: AccessServiceEnum.PROJECT, icon: FiAlertTriangle },
